...
def test_log_loss_is_minus_log_of_actual():
    assert log_loss((0.25, 0.25, 0.5), 0, 1) == pytest.approx(-math.log(0.5))
    assert log_loss((1.0, 0.0, 0.0), 3, 0) == pytest.approx(0.0)


def test_brier_multiclass():
    # Acierto perfecto: (1,0,0) vs home -> 0
    assert brier((1.0, 0.0, 0.0), 2, 0) == pytest.approx(0.0)
    # Uniforme vs home: sum (1/3... )^2 con one-hot
    expected = ((1 - 1 / 3) ** 2) + (1 / 3) ** 2 + (1 / 3) ** 2
    assert brier((1 / 3, 1 / 3, 1 / 3), 2, 0) == pytest.approx(expected)
...
def test_evaluate_model_aggregates():
    ...
    assert report.log_loss == pytest.approx((-math.log(0.8) - math.log(0.3)) / 2)
    assert 0.0 < report.brier <= (2.0 / 3)
